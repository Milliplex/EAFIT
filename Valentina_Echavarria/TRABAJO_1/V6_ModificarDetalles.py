...
while True:
    ...
    (5) Modificar Detalles de un Producto
    (6) Ver Productos
    ...
    # Modificar Detalles de un Productos
    elif opcion == 5:
        buscador = input('Ingrese el Producto que quiere Modificar: ')
        if buscador in productos or alimentos_orga or productos_hc or alterna_sos or productos_limE:
            posicion = productos.index(buscador)

            if buscador in deta_alimentos_orga:
                deta_alimentos_orga = input('Ingrese los nuevos detalles del producto: ')
                alimentos_orga[posicion] = deta_alimentos_orga

            elif buscador in deta_productos_hc:
                deta_productos_hc = input('Ingrese los nuevos detalles del producto: ')
                productos_hc[posicion] = deta_productos_hc
                
            elif buscador in deta_alterna_sos:
                deta_alterna_sos = input('Ingrese los nuevos detalles del producto: ')
                alterna_sos[posicion] = deta_alterna_sos

            elif buscador in deta_productos_limE:
                deta_productos_limE = input('Ingrese los nuevos detalles del producto: ')
                productos_limE[posicion] = deta_productos_limE
